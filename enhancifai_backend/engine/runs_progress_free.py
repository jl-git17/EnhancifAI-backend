...
class RunsProgressFree:
    ...
    def add_run(self, run_id: int, total_rows: int):
        ...
        RunsDbCore.insert_run_details(run_id, json.dumps(run_details), free=True)

    def update_progress(self, run_id: int, rows_processed: int):
        """
        Updates the progress of a specific run.
        """
        run_details = RunsDbCore.get_run_details(run_id, free=True)['run_details']
        if RunsDbCore.is_run_cancelled(run_id, free=True):
    ...
    def update_details(self, run_id: int, details: Dict):
        ...
        if run_details['status'] != 'completed' and RunsDbCore.is_run_cancelled(run_id, free=True) is False:
    ...
    def check_status(self, run_id: int) -> Optional[Dict]:
        ...
        """
        _run_details = RunsDbCore.get_run_details(run_id, free=True)
...
runs_progress_free = RunsProgressFree()
