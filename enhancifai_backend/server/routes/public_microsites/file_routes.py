...
def get_cache_file_path(session_id, filename):
    ensure_cache_directory()
    user_dir = os.path.join(CACHE_DIRECTORY, str(session_id))
    os.makedirs(user_dir, exist_ok=True)
    return os.path.join(user_dir, filename)

def save_to_cache(file_path, session_id, filename):
    cache_path = get_cache_file_path(session_id, filename)
    os.makedirs(os.path.dirname(cache_path), exist_ok=True)
    shutil.copy(file_path, cache_path)
    return cache_path

def get_from_cache(session_id, filename):
    cache_path = get_cache_file_path(session_id, filename)
...
async def get_cached_file(
    ...
    cache_path = get_from_cache(session_id, req.filename)
...
async def add_cached_file(
    ...
    # Save file to cache
    save_to_cache(temp_data_file_path, session_id, data_file.filename)
