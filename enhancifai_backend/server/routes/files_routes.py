...
from enhancifai_backend.server.utils import CACHE_DIRECTORY, CACHE_DIRECTORY_FREE, get_current_user_id, verify_secret_key


router = APIRouter()

def ensure_cache_directory(free: bool = False):
    if free:
        if not os.path.exists(CACHE_DIRECTORY_FREE):
            os.makedirs(CACHE_DIRECTORY_FREE)
    else:
        if not os.path.exists(CACHE_DIRECTORY):
            os.makedirs(CACHE_DIRECTORY)

def get_cache_file_path(user_id, filename, free: bool = False):
    ensure_cache_directory(free)
    user_dir = os.path.join(CACHE_DIRECTORY_FREE if free else CACHE_DIRECTORY, str(user_id))
    os.makedirs(user_dir, exist_ok=True)
    return os.path.join(user_dir, filename)

def save_to_cache(file_path, user_id, filename, free: bool = False):
    cache_path = get_cache_file_path(user_id, filename, free)
    os.makedirs(os.path.dirname(cache_path), exist_ok=True)
    shutil.copy(file_path, cache_path)
    return cache_path

def get_from_cache(user_id, filename, free: bool = False):
    cache_path = get_cache_file_path(user_id, filename, free)
